...
1. At the start of every user-submitted task, call `set_codex_light` with `status: "red"`.
2. Before any request for authorization, permission, or approval, call `set_codex_light` with `status: "yellow"`.
3. Before the final response, call `set_codex_light` with `status: "green"`.
4. Call `get_codex_light_status` when the user asks whether the floating window is connected or what it currently shows.

Do not send a custom `message` unless the user explicitly asks for one. The floating window chooses Chinese or English text from its right-click language setting.
